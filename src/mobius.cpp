...
void AppendIncludeDirectory(std::vector<std::string>& cflags, const std::string& directory) {
    cflags.emplace_back("-I" + directory);
}
...
namespace mobius {
std::string BuildDir() {
    return gMobius.buildDir;
}
} // namespace mobius

int main(int argc, const char** argv) {
    if (argc != 2) {
        Fatal("usage: mobius [builddir]\n");
    }
    ...
    if (Run({cxx,
            ...
            "-MD", "-MF", "build.d",
            "build.cpp", "-o", buildLib.c_str()}) != 0) {
        Fatal("Failed to compile build.cpp run c++\n");
    }
    ...
    NinjaVariable(ninja, "builddir", buildDir);
    ...
    for (const auto& target : project.targets) {
        ...
        if (target.type == TargetType::Executable) {
            NinjaBuild(ninja, "$builddir/" + target.name, "link", objectFiles, extraBuildVars);
        } else if (target.type == TargetType::StaticLibrary) {
            NinjaBuild(ninja, "$builddir/" + target.name + ".a", "ar", objectFiles, extraBuildVars);
        } else if (target.type == TargetType::SharedLibrary) {
            NinjaBuild(ninja, "$builddir/" + target.name + ".so", "link", objectFiles, extraBuildVars);
        }
        NinjaNewline(ninja);
    }

    // #SoMeta
    NinjaRule(ninja, "mobius", "mobius $builddir", {{"generator", {"1"}},
             {"depfile", {"build.d"}}, {"deps", {"gcc"}}});
    NinjaBuild(ninja, "build.ninja", "mobius", {"| $root/build.cpp"});

    fprintf(ninja, "\n");
    fclose(ninja);

    printf("Wrote build.ninja\n");
}
